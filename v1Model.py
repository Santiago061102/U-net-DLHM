...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, dropout=False):
        ...
        if dropout:
            layers.append(nn.Dropout2d(p=0.4, inplace=True))
...
class UpSample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, attn=False):
        super().__init__()
...
class UNet(nn.Module):
    def __init__(self, filters, kernel_size=3, attn = False):
        ...
        self.up_convolution1 = UpSample(filters*16, filters*8, kernel_size, attn=True)
        self.up_convolution2 = UpSample(filters*8, filters*4, kernel_size, attn=True)
        self.up_convolution3 = UpSample(filters*4, filters*2, kernel_size, attn=False)
        self.up_convolution4 = UpSample(filters*2, filters, kernel_size, attn=False)
